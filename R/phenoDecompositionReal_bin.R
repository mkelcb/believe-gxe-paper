...
lastArg=5
numPops=(length(args)-lastArg)/2 # BELIEVE UK_SEA 0.02 0.03

pops = c()
for (i in (lastArg+1):(lastArg+numPops) ) { # loop through the rest of the arguments, where each is supposed to be a pop label
  pops = c(pops,args[i] )
}

prevalences = c()
for (i in (lastArg+numPops+1):length(args) ) { # loop through the rest of the arguments, where each is supposed to be a prevalence
  prevalences = c(prevalences,as.numeric(args[i]) )
}

print(paste0("mean(prevalences): ", mean(prevalences)))
...
print(paste0("Starting performAllTests, K_standard: ", dim(K_standard), " / K_subpops[[1]]: ", dim(K_subpops[[1]])) )
  print(paste0("nrow(famData): ",nrow(famData) , " / nrow(fam_pheno): ",nrow(fam_pheno) ))

truthVg = performAllTests_bin(K_standard, famData, outLoc, envIndicator, fam_pheno, mean(prevalences))
  ...
  for (k in 1:length(X_pops_indices)) {
   ...
   # Xz = scale(X_all[popIndicecs,])
    ...
    #print(paste0("SUB test nrow(famData): ",nrow(famData_sub) , " / nrow(fam_pheno): ",nrow(fam_pheno[popIndicecs,]) ))
    truthVg_subpop = estimate_VgVe_bin(K_sub, famData_sub, outLoc_sub, fam_pheno[popIndicecs,], prevalences[k])
